...
export default class WeatherPreferences extends ExtensionPreferences {
    ...
    _buildUnitsGroup(page, gweatherSettings) {
        ...
        group.add(this._enumRow(_('Temperature'), gweatherSettings, 'temperature-unit', [
        ...
        group.add(this._enumRow(_('Wind Speed'), gweatherSettings, 'speed-unit', [
        ...
        group.add(this._enumRow(_('Pressure'), gweatherSettings, 'pressure-unit', [
        ...
        group.add(this._enumRow(_('Distance'), gweatherSettings, 'distance-unit', [
    ...
    _buildPanelGroup(page, settings) {
        ...
        group.add(this._enumRow(_('Position in Panel'), settings, 'position-in-panel', [
            [0, _('Center')], [1, _('Right')], [2, _('Left')],
        ]));
        group.add(this._boolChoiceRow(_('Wind Direction'), settings, 'wind-direction',
            [_('Letters'), _('Arrows')]));
    ...
    _enumRow(title, settings, key, options) {
        ...
        settings.connect(`changed::${key}`, () => (row.selected = indexOf(settings.get_enum(key))));
        return row;
    }

    // Presents a boolean setting as a two-item choice (used for wind-direction).
    _boolChoiceRow(title, settings, key, [falseLabel, trueLabel]) {
        ...
        settings.connect(`changed::${key}`, () => (row.selected = Number(settings.get_boolean(key))));
        return row;
    }
}
